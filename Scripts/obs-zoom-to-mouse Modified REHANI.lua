...
local follow_safezone_sensitivity = 2
...
function is_display_capture(source_to_check)
    if source_to_check ~= nil then
        ...
        local dc_info = get_dc_info()
        if dc_info ~= nil then
            local id = obs.obs_source_get_id(source_to_check)
            if id == dc_info.source_id or id == "wgc_capture" or id == "monitor_capture" then
                return true
            end
        end
    end
...
function release_sceneitem()
    ...
    if sceneitem ~= nil then
        ...
        if sceneitem_info_orig ~= nil then
            log("Zoom: Resetting transform info to original")
...
function refresh_sceneitem(find_newest)
    ...
    if sceneitem ~= nil then
        ...
        if crop_filter == nil then
            log("Zoom: Creating new crop filter '" .. CROP_FILTER_NAME .. "'")
            ...
            log("Zoom: Using existing crop filter")
...
function on_toggle_zoom(pressed)
    if pressed then
        ...
        log("Zoom: Hotkey pressed at " .. m.x .. "," .. m.y .. ". State: " .. tostring(zoom_state))
...
function set_crop_settings(crop)
    if crop_filter ~= nil and crop_filter_settings ~= nil then
        ...
        local lx = math.floor(crop.x)
        local ly = math.floor(crop.y)
        local lw = math.floor(crop.w)
        local lh = math.floor(crop.h)
        obs.obs_data_set_int(crop_filter_settings, "left", lx)
        obs.obs_data_set_int(crop_filter_settings, "top", ly)
        obs.obs_data_set_int(crop_filter_settings, "cx", lw)
        obs.obs_data_set_int(crop_filter_settings, "cy", lh)
        obs.obs_source_update(crop_filter, crop_filter_settings)
        -- log("Zoom: Set crop " .. lx .. "," .. ly .. " " .. lw .. "x" .. lh)
    end
end
...
function on_print_help(props, prop)
    local help = "\n----------------------------------------------------\n" ..
...
function script_properties()
    ...
    obs.obs_properties_add_button(props, "test_zoom_in", "Test Zoom In", function() 
        if zoom_state == ZoomState.None then
            on_toggle_zoom(true)
        end
        return true 
    end)
    obs.obs_properties_add_button(props, "test_zoom_out", "Test Zoom Out", function() 
        if zoom_state == ZoomState.ZoomedIn then
            on_toggle_zoom(true)
        end
        return true 
